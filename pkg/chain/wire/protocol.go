...
const (
	...
	// MultipleAddressVersion is the protocol version which added multiple addresses per message (pver >=
	// MultipleAddressVersion).
	MultipleAddressVersion uint32 = 209
	// NetAddressTimeVersion is the protocol version which added the timestamp field (pver >= NetAddressTimeVersion).
	NetAddressTimeVersion uint32 = 31402
	// BIP0031Version is the protocol version AFTER which a pong message and nonce field in ping were added (pver >
	// BIP0031Version).
	...
	// BIP0037Version is the protocol version which added new connection bloom filtering related messages and extended
	// the version message with a relay flag (pver >= BIP0037Version).
...
// Constants used to indicate the message bitcoin network. They can also be used to seek to the next message when a
// stream's state is unknown, but this package does not provide that functionality since it's generally a better idea to
